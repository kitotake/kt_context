-- =============================================
-- PATCH cursor.lua pour kt_interact
-- À REMPLACER dans client/alts_client/cursor.lua
-- =============================================
-- Ce fichier étend OpenGeneralContextMenu et OpenContextForEntity
-- pour injecter les interactions kt_interact disponibles.
-- =============================================
...
-- ─── Helper : injection items kt_interact ────────────────────────────────────
local function _injectInteractItems(items, context)
    if not KtInteract_GetMenuItems then return items end

    local interactItems = KtInteract_GetMenuItems(context)
    if not interactItems or #interactItems == 0 then return items end

    -- Divider avant les interactions kt_interact
    table.insert(items, { id = '_div_kt_interact', divider = true, label = '' })
    table.insert(items, {
        id       = '_kt_interact_header',
        label    = '🎯 Interactions',
        icon     = 'Crosshair',
        disabled = true,
    })

    for _, iitem in ipairs(interactItems) do
        table.insert(items, iitem)
    end
...
function OpenContextForEntity(entity, entityType, x, y)
    ...
    -- Contexte pour kt_interact
    local interactContext = {
        entity   = entity,
    ...
    if entityType == 1 then
        if IsPedAPlayer(entity) then
            ...
            if localId ~= -1 then
                ...
                interactContext.type     = 'ped'
                interactContext.isPlayer = true
            else
    ...
    elseif entityType == 2 then
        ...
        interactContext.type = 'vehicle'
    elseif entityType == 3 then
        items = BuildPropMenu(entity, isAdmin)
        interactContext.type = 'object'
    else
        OpenGeneralContextMenu(x, y)
        return
    end

    -- Injection des interactions kt_interact selon l'entité
    items = _injectInteractItems(items, interactContext)
...
function OpenGeneralContextMenu(x, y)
    ...
    -- ── Interactions kt_interact à portée ─────────────────────────────────
    local interactItems = nil
    if KtInteract_GetGeneralMenuItems then
        interactItems = KtInteract_GetGeneralMenuItems()
    end

    if interactItems and #interactItems > 0 then
        -- Sous-menu groupé si > 3 interactions, sinon on les intègre directement
        if #interactItems > 3 then
            table.insert(items, {
                id      = 'kt_interact_nearby',
                label   = ('Interactions (%d)'):format(#interactItems),
                icon    = 'Crosshair',
                submenu = interactItems,
            })
        else
            table.insert(items, { id = '_div_kt_interact', divider = true, label = '' })
            for _, iitem in ipairs(interactItems) do

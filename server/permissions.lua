-- =============================================
-- PERMISSIONS SERVEUR — v3.2
-- FIX : playerConnecting + onServerResourceStart + rate limiting
-- =============================================
Permissions = {}
...
function IsRateLimited(src, action, ms)
    ...
    if (nowMs - lastMs) < ms then return true end
...
function Permissions.IsAdmin(src)  return Permissions.Has(src, 'admin') end
function Permissions.IsStaff(src)  return Permissions.Has(src, 'staff') end
function Permissions.GetRole(src)  return cache[src] or 'user' end

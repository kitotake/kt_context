...
const DEV_ITEMS: MenuItem[] = [
  ...
  { id: '_div1', divider: true, label: '', disabled: true },
  {
    id: 'admin_section',
    label: '⚡ Options Admin',
    icon: 'ShieldAlert',
...
const App: FC = () => {
  ...
  /* En mode FiveM on cache le body par défaut; il sera rendu visible par le Lua */
  useEffect(() => {
    if (isEnvBrowser()) {
  ...
  return (
    <>
      ...
      {isEnvBrowser() && (
        ...
        >
          <h1 style={{ color: '#f1f5f9', fontSize: 32, fontWeight: 800, letterSpacing: -1 }}>
          ...
          <p style={{ color: '#475569', fontSize: 13, marginTop: 8 }}>
            En jeu : maintenez <kbd style={{ background: '#1e293b', color: '#94a3b8', padding: '2px 6px', borderRadius: 4, border: '1px solid #334155' }}>ALT</kbd> puis cliquez sur une entité
          </p>
        </div>
      )}
